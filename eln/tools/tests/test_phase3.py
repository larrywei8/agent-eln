...
ELN = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
FIXTURES = os.path.join(os.path.dirname(__file__), "fixtures", "phase3")


def _sandbox(with_workspace=False):
    """Copy ELN to a tmp directory. When with_workspace=True, also create tmp/wiki/summaries/."""
    tmp_ws = tempfile.mkdtemp(prefix="eln-p3-ws-")
    eln_root = os.path.join(tmp_ws, "eln")
    os.makedirs(eln_root)
    for name in ("tools", "resources", "experiments", "protocols", "pipelines",
                 "scripts", "skills", "ideas", "projects",
                 "meetings", "templates", "references"):
        src = os.path.join(ELN, name)
        if os.path.isdir(src):
            shutil.copytree(src, os.path.join(eln_root, name),
                            ignore=shutil.ignore_patterns("__pycache__", "*.pyc"))
    os.makedirs(os.path.join(eln_root, "literature"), exist_ok=True)  # empty, each test starts from scratch
    os.makedirs(os.path.join(eln_root, "index"), exist_ok=True)
    if with_workspace:
        os.makedirs(os.path.join(tmp_ws, "wiki", "summaries"), exist_ok=True)
    return tmp_ws, eln_root
...
class DoiIngestTests(unittest.TestCase):
    ...
    def test_1_doi_stub_happy_path(self):
        ...
        cards = [f for f in os.listdir(os.path.join(self.root, "literature")) if f.startswith("LIT-0001")]
        self.assertEqual(len(cards), 1)
        with open(os.path.join(self.root, "literature", cards[0])) as f:
            content = f.read()
    ...
    def test_2_doi_dedup(self):
        ...
        self.assertIn("LIT-0001", r.stdout)
        cards = [f for f in os.listdir(os.path.join(self.root, "literature")) if f.startswith("LIT-")]
    ...
    def test_3_doi_normalization(self):
        ...
        r = _run(self.root, "lit_from_doi.py", "https://doi.org/10.1038/S41586-026-XYZ",
                 "--stub", "--no-index", "--by", "t")
        card_files = [f for f in os.listdir(os.path.join(self.root, "literature")) if f.startswith("LIT-")]
        with open(os.path.join(self.root, "literature", card_files[0])) as f:
            content = f.read()
...
class WikiSyncTests(unittest.TestCase):
    ...
    def test_7_wiki_sync_forward(self):
        ...
        # find card
        cards = [f for f in os.listdir(os.path.join(self.root, "literature")) if f.startswith("LIT-")]
        card_path = os.path.join(self.root, "literature", cards[0])
        # create wiki summary
...
class HealthTests(unittest.TestCase):
    ...
    def test_8_health_9_dimensions(self):
        ...
        os.makedirs(os.path.join(self.root, "literature"), exist_ok=True)
        card = """---
id: LIT-0001
type: literature
...
Just a stub. [TBD]
"""
        with open(os.path.join(self.root, "literature", "LIT-0001-broken.md"), "w") as f:
            f.write(card)
