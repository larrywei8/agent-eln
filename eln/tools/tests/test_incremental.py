...
GOLDEN = os.path.join(ELN, "tools", "tests", "golden")
...
def _sandbox():
    """Copy ELN's key subdirectories to a tmp directory, return that root."""
    tmp = tempfile.mkdtemp(prefix="eln-test-")
    for name in ("tools", "resources", "experiments", "protocols", "pipelines",
                 "scripts", "skills", "literature", "ideas", "projects",
                 "meetings", "templates"):
    ...
    exp_dir = os.path.join(tmp, "experiments", "2099", "2099-01-01")
