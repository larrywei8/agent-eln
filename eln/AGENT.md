...
You are now inside an **AI-native Electronic Lab Notebook (AI-native ELN)**. It is a
...
## 1. Directory map
- `resources/`   frequently-reused resources: plasmids, oligos, dna, reagents, antibodies,
                 samples, mice, cell-lines, instruments, viruses
- `protocols/`   wet-lab SOPs        `pipelines/` dry-lab analysis pipelines
- `scripts/`     reusable code        `skills/`    AI skills (prompt + code)
- `experiments/` daily experiment notes (wet/dry) — `experiments/<year>/<date>/EXP-XXXX.md` +
                 an optional **same-named artifacts folder** `EXP-XXXX/` (see § 3 "experiment artifacts")
- `projects/`    project layer (PRJ): group cross-day experiments/ideas/data/literature into a research line
- `literature/`  literature library + reading notes
- `ideas/`       sparks of ideas          `meetings/`  meeting notes + PPT transcripts
- `templates/`   template for each record type    `tools/` indexing/validation/scaffold scripts
...
- Tabular queries: read `index/<type>.csv` (e.g. plasmids.csv columns = backbone/resistance/insert)
  or the grouping tables resources/datasets/experiments.csv.
...
Files are **always physically filed by type into resources/** (plasmid → plasmids/,
...
side-by-side:
```
resources/plasmids/PLA-0042-.../ card.md  sequence.fasta  map.gb  map.png
```
